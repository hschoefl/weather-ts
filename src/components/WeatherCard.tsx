    ...
    <div className="flex flex-col border-2 border-gray-300 p-8 rounded-lg gap-2 shadow-2xl w-s max-w-s items-center bg-white">
      ...
      <div className="grid grid-cols-2 gap-6 justify-items-left">
        ...
        <div className="grid grid-cols-12 justify-items-start">
          <img src={imageSunrise} alt="sunrise icon" className="w-6 h-6 col-span-4" />
          <p className="text-2xl col-span-8">{convertGMTToLocalTime(sunrise)} Uhr</p>
        </div>

        {/* Sunset */}
        <div className="grid grid-cols-12 justify-items-start text-2xl">
          <img src={imageSunset} alt="sunset icon" className="w-6 h-6 col-span-4" />
          <p className="text-2xl col-span-8">{convertGMTToLocalTime(sunset)} Uhr</p>
        </div>
        ...
        <div className="grid grid-cols-12 justify-items-start text-2xl">
          ...
          <p className="col-span-8">{getWindDirection(wind_direction_10m_dominant)}</p>
        </div>

        {/* Humidity */}
        <div className={`grid grid-cols-12 justify-items-start text-2xl`}>
          <img src={imageHumidity} alt="Humidity icon" className="w-8 h-8 col-span-4" />
          <p className="text-2xl col-span-8">{relative_humidity_2m_max} %</p>
        </div>

        {/* UV Index  */}
        <div className={`grid grid-cols-12 justify-items-start text-2xl`}>
          <img src={imageUVIndex} alt="UV Index icon" className="w-8 h-8 col-span-4" />
          <p className={`text-2xl rounded-2xl col-span-8 bg-[${getUVIndexColor(uv_index_max)}]`}>{uv_index_max}</p>
        </div>
      </div>
      {/* End of grid */}
    </div>
